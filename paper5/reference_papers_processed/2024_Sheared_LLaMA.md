---
id: "2024_Sheared_LLaMA"
title: "Sheared LLaMA: Accelerating Language Model Pre-training via Structured Pruning"
authors: ["Mengzhou Xia", "Tianyu Gao", "Zhiyuan Zeng", "Danqi Chen"]
year: 2024
venue: "ICLR 2024"
publication_status: "FORMALLY PUBLISHED"
category: "Structured Compression / Post-Training Adaptation"
source_pdf: "../reference_papers_origin/2024_Sheared_LLaMA.pdf"
paper_url: "https://openreview.net/forum?id=09iOdaeOzp"
pdf_url: "https://arxiv.org/pdf/2310.06694"
...
**Why this paper matters for low-cost post-training LLM NAS:** It makes target depth, head, hidden, and FFN dimensions explicit architecture variables and studies low-compute recovery from a pretrained LLM.

**Primary record:** [https://openreview.net/forum?id=09iOdaeOzp](https://openreview.net/forum?id=09iOdaeOzp)

**Local source:** [2024_Sheared_LLaMA.pdf](../reference_papers_origin/2024_Sheared_LLaMA.pdf)
...
_{_ mengzhou,tianyug,danqic _}_ @cs.princeton.edu
...
larger models. Our approach employs two key techniques: (1) _targeted structured_
_pruning_, which prunes a larger model to a specified target shape by removing
layers, heads, and intermediate and hidden dimensions in an end-to-end manner,
and (2) _dynamic batch loading_, which dynamically updates the composition of
sampled data in each training batch based on varying losses across different domains. We demonstrate the efficacy of our approach by presenting the **Sheared-**
**LLaMA** series, pruning the LLaMA2-7B model down to 1.3B and 2.7B parameters. Sheared-LLaMA models outperform state-of-the-art open-source models

    - f equivalent sizes, such as Pythia, INCITE, OpenLLaMA and the concurrent
...
1
[Please find our code and models at https://github.com/princeton-nlp/LLM-Shearing.](https://github.com/princeton-nlp/LLM-Shearing)
...
dubbed _targeted structured pruning_, which
prunes a source model to a specified target architecture. The target architecture
is determined by leveraging the configurations of existing pre-trained models. Our
pruning approach searches for substructures within the source model that maximally preserve performance while adhering to the given constraints.


- We devise a _dynamic batch loading_ algorithm that loads training data from each
...
Caption: Sheared-LLaMA-2.7B surpasses a series of open-source models at a similar scale and
...
- nly requires 1/32 (3%) of budget to achieve onpar performance with OpenLLaMA-3B-v2.
...
Given an existing large model _MS_ (the _source_ model), we study how to efficiently produce a smaller,
strong model _MT_ (the _target_ model). We consider this as a two-stage process: (1) Pruning _MS_ into
_MT_ . This reduces the number of parameters but inevitably incurs a performance drop. (2) Continue
pre-training _MT_ with a standard language modeling objective to reach a target performance. While
...
Caption: _Targeted structured pruning_ produces a compact and dense model of a pre-specified shape.

Meaning: The figure supports the method or empirical discussion described by this caption; consult the local source PDF for the visual encoding.
Light colors indicate pruned substructures. Masking variables _z_ are learned to control whether a
substructure is pruned ( _z_ = 0) or retained ( _z_ = 1).
...
expressivity and inference efficiency. For example, we use the INCITE-Base-3B architecture (TogetherAI, 2023a) as the target structure when producing a 2 _._ 7B model.
...
_MS_ has a hidden state dimension of _dS_, _HS_ heads in each MHA, and an intermediate dimension of
...
Pruning masks _z_ [layer] _∈_ R _[L][S]_ _z_ [hidden] _∈_ R _[d][S]_ _z_ [head] _∈_ R _[H][S]_ ( _×LS_ ) _z_ [int] _∈_ R _[m][S]_ ( _×LS_ )


Each mask variable controls whether the associated substructure is pruned or retained. For example,
we remove a layer if its corresponding _z_ [layer] = 0. Figure 2 illustrates an example of how the pruning
...
maximizing performance. [2] Following the _ℓ_ 0 regularization approach (Louizos et al., 2018), we
parametrize the pruning masks to model hard concrete distributions. These distributions have support on [0 _,_ 1] but concentrate their probability mass at 0 or 1, enabling discrete prune or retain
decisions. While prior work usually control for a target sparsity (Wang et al., 2020; Xia et al.,
2022), we use a pair of Lagrange multipliers to impose constraints on the pruned model shape directly. For example, for a target number of heads _HT_ (and we use _LT_, _dT_, and _mT_ to represent the
...
˜ 2
_L_ [head] ( _λ, ϕ, z_ ) = _λ_ [head] _·_ _z_ [head] _−_ _HT_ + _ϕ_ [head] _·_ _z_ [head] _−_ _HT_ _._

       - �        -        - �

Similar constraints are applied to pruning other substructures. Overall, we jointly optimize the
model weights and pruning masks by a min-max objective min _θ,z_ max _λ,ϕ L_ prune( _θ, z, λ, ϕ_ ):



_LS_

- _L_ ˜ [int] _j_ + _L_ [˜][layer] + _L_ [˜][hidden] _,_

_j_ =1



_L_ prune( _θ, z, λ, ϕ_ ) = _L_ ( _θ, z_ ) +
...
where _L_ ( _θ, z_ ) is the language modeling loss computed with the masked model weights. This objective will produce a pruned model with the target shape. Ideally, running this pruning algorithm on
a large amount of data will directly produce a strong compact model. In practice, the pruning stage
is expensive (roughly 5 _×_ slower compared to standard LM training), and we find that the learned
...
**Require** : Training data of _k_ domains _D_ 1 _, D_ 2 _, · · ·, Dk_, validation data _D_ 1 [val] _[, D]_ 2 [val] _[,][ · · ·][, D]_ _k_ [val][,]
initial data loading weights _w_ 0 _∈_ R _[k]_, reference loss _ℓ_ ref _∈_ R _[k]_, LM loss _L_ - r pruning loss
_L_ prune, training steps _T_, evaluation per _m_ steps, model parameters _θ_ ( _θ, z, ϕ, λ_ for pruning)


**for** _t_ = 1 _, · · ·, T_ **do**

**if** _t_ mod _m_ = 0 **then**

_ℓt_ [ _i_ ] _←L_ ( _θ, z, Di_ [val][)][ if] _[ pruning]_ [ else] _[ L]_ [(] _[θ, D]_ _i_ [val][)]
∆ _t_ [ _i_ ] _←_ max _{ℓt_ [ _i_ ] _−_ _ℓ_ ref [ _i_ ] _,_ 0 _}_ _▷_ Calculate loss difference
_wt ←_ UpdateWeight( _wt−m,_ ∆ _t_ ) _▷_ Update data loading proportion
**end**
Sample a batch of data _B_ from _D_ 1 _, D_ 2 _, · · ·, Dk_ with proportion _wt_ ;
**if** _pruning_ **then**

Update _θ, z, ϕ, λ_ with _L_ prune( _θ, z, ϕ, λ_ ) on _B_
**else**

Update _θ_ with _L_ ( _θ, B_ )
**end**

**end**


**Subroutine** UpdateWeight( _w,_ ∆)

_α ←_ _w ·_ exp (∆) _▷_ Calculate the unnormalized weights
_w ←_ ~~�~~ _iα_ _[α]_ [[] _[i]_ []] **[return]** _[ w]_ _▷_ Renormalize the data loading proportion
...
To be more specific, we begin by fitting a _scaling function_ (Hoffmann et al., 2022; details in Appendix B) on the series of LLaMA2 models for each domain. Using this function, we predict the
loss of a hypothetical 1.3B LLaMA2 model if it were trained from scratch on the same data. We
then compare these estimated _reference losses_ to the losses of our pruned model after continued
...
- verall loss is seemingly low, as demonstrated later in Section 4.1.


Inspired by recent work (Xie et al., 2023), we propose _dynamic batch loading_, an efficient algorithm
...
**Problem setup.** The pre-training data comprises of _k_ domains _D_ 1 _, D_ 2 _, · · ·, Dk_ and we have a held
- ut validation dataset for each domain, denoted as _Di_ [val][. At each training step] _[ t]_ [, a proportion] _[ w][t]_ [[] _[i]_ []]

- f the data comes from domain _Di_ . We set a reference validation loss _ℓ_ ref ( _Di_ ) for each domain and
...
- bserved a similar phenomenon, indicating that this is a universal issue unrelated to specific pre-training data.
...
steps, we evaluate the model to get the validation loss _ℓt_ (step _t_ ) on _D_ [val], and update _wt_ based on the
difference ∆ _t_ ( _Di_ ) between _ℓ_ ref [ _i_ ] and _ℓt_ [ _i_ ] on each domain. The update rule is exponential ascent
following Xie et al. (2023),


_αt_
_αt_ = _wt−m ·_ exp(∆ _t_ ); _wt_ =

~~�~~ _i_ _[α][t]_ [[] _[i]_ []] _[.]_


We apply dynamic batch loading to both the pruning stage and the continued pre-training stage. For
pruning, we use the original pre-training data’s domain weights as _w_ 0. For continued pre-training,
we use the final weights from the pruning stage as _w_ 0. Dynamic batch loading is an on-the-fly
...
- verhead to standard training. This approach differs from Xie et al. (2023), which requires a complex
...
as the reference (denoted as _scaling reference_ ). We also experiment with an alternative where we
directly use the source model’s domain validation loss as the reference (denoted as _source reference_ ).
...
**Data.** As the training data for LLaMA2 is not pub- Table 1:
licly accessible, we use RedPajama (TogetherAI, 2023b),
which is a replicated pre-training dataset of the LLaMA1 - ther models.
models (Touvron et al., 2023a), for pruning and
continued-pretraining. This dataset encompasses training **Model**
data from seven domains: CommonCrawl, C4, Github, LLaMA1
Wikipedia, Books, ArXiv, and StackExchange. We con- LLaMA2
struct a held-out validation set with 2 million tokens OPT
(equivalent to 500 sequences of 4,096 tokens) for each do- Pythia

INCITE-Base

main. We allocate 0.4 billion tokens for the pruning phase OpenLLaMA v1
and 50 billion tokens for the continued pre-training pro- OpenLLaMA v2
cess. Following the conventions of LLaMA2, we main- TinyLlama
tain a sequence length of 4,096 tokens. Table 1 provides Sheared-LLaMA RedPajama
...
OpenLLaMA v2 OpenLLaMA data [6] 1T
TinyLlama TinyLlama data [7] 3T
Sheared-LLaMA RedPajama 50B



4
Please find results on LLaMA1 models in Appendix F.6 and Pythia models in Appendix F.5.
...
tasks. The shot number used is noted in parentheses, with 0-shot if not specified. Models with _†_ use
...
LLaMA2-7B (2T) _[†]_ 93.7 78.1 69.3 76.4 53.0 78.6


OPT-1.3B (300B) _[†]_ 84.3 71.7 **59.6** 57.0 29.7 54.5
Pythia-1.4B (300B) _[†]_ 86.4 70.9 57.4 60.7 31.2 53.0
TinyLlama-1.1B (3T) _[†]_ **88.9** 73.3 58.8 55.3 30.1 60.3
Sheared-LLaMA-1.3B (50B) 87.3 **73.4** 57.9 **61.5** **33.5** **60.7**


OPT-2.7B (300B) _[†]_ 85.8 73.7 60.8 60.8 34.0 61.5
Pythia-2.8B (300B) _[†]_ 88.3 74.0 59.7 64.4 36.4 60.8
INCITE-Base-3B (800B) 90.7 74.6 63.5 **67.7** 40.2 64.8
Open-LLaMA-3B-v1 (1T) 91.3 73.7 61.5 67.6 39.6 62.6
Open-LLaMA-3B-v2 (1T) _[†]_ **91.8** **76.2** 63.5 66.5 39.0 67.6
...
LLaMA2-7B (2T) _[†]_ 30.7 82.1 28.8 73.9 46.6 64.6


OPT-1.3B (300B) _[†]_ 26.9 57.5 58.0 6.9 24.7 48.2
Pythia-1.4B (300B) _[†]_ **27.3** 57.4 **61.6** 6.2 **25.7** 48.9
TinyLlama-1.1B (3T) _[†]_ 26.3 60.9 58.8 **12.1** 25.5 50.0
Sheared-LLaMA-1.3B (50B) 26.9 **64.0** 61.0 9.6 **25.7** **51.0**


OPT-2.7B (300B) _[†]_ 26.0 63.4 63.6 10.1 25.9 51.4
Pythia-2.8B (300B) _[†]_ 28.0 66.0 64.7 9.0 26.9 52.5
INCITE-Base-3B (800B) 27.7 65.9 65.3 14.9 **27.0** 54.7
Open-LLaMA-3B-v1 (1T) 28.4 70.0 65.4 **18.6** **27.0** 55.1
Open-LLaMA-3B-v2 (1T) _[†]_ 28.1 69.6 66.5 17.1 26.9 55.7
...
- n an extensive suite of downstream tasks: (1) We follow Pythia and LLaMA2 to report the 0-shot
...
Sheared-LLaMA-1.3B Pythia-1.4B


Sheared-LLaMA-2.7B Open-LLaMA-v1-3B



Sheared-LLaMA-2.7B INCITE-Base-3B


Sheared-LLaMA-2.7B Open-LLaMA-v2-3B



### Figure 3

Caption: Sheared-LLaMAs outperform Pythia-1.4B, INCITE-Base-3B, OpenLLaMA-3B-v1 and

Meaning: The figure supports the method or empirical discussion described by this caption; consult the local source PDF for the visual encoding.
OpenLLaMA-3B-v2 in instruction tuning.


- n 3T tokens). Sheared-LLaMA-2.7B outperforms INCITE-Base-3B (pre-trained on 800B RedPajama tokens), OpenLLaMA-3B-v1 (pre-trained on 1T RedPajama tokens), and OpenLLaMA-3Bv2 (trained on 1T tokens from RedPajama, RefinedWeb, and StarCoder). The most noteworthy
...
Caption: shows the difference between our model’s loss (with both original and dynamic batch loading) and the reference loss, estimated by fitting a scaling function to a hypothetical 1.3B parameter
...
RedPajama (Original) 67 _._ 0% 4 _._ 5% 4 _._ 5% 2 _._ 0% 4 _._ 5% 2 _._ 5% 15 _._ 0%
Dynamic Batch Loading 36 _._ 1% 0 _._ 8% 9 _._ 1% 1 _._ 0% 3 _._ 1% 0 _._ 7% 49 _._ 2%
...
Caption: Loss difference between the pruned

Meaning: The figure supports the method or empirical discussion described by this caption; consult the local source PDF for the visual encoding.
model (1.3B) and estimated reference loss, with

- riginal vs. dynamic batch loading.



### Figure 5

Caption: Downstream task performance of
...
**Layer Config** **PPL** _↓_ **Speed** _↑_ **Layer Config** **PPL** _↓_ **Speed** _↑_


9.1 51 7.0 37
1.3B [CoFiPruning] 2.7B [CoFiPruning]
...
- f 5B tokens. Table 5 shows that when controlling the total amount of tokens, increasing the pruning budget consistently improves perplexity. However, since pruning is
...
- nes are often overparameterized and can be pruned significantly with minimal impact on performance (Han et al., 2016; Wen et al., 2016; Liu et al., 2017; Luo et al., 2017; Cai et al., 2019; Deng
...
**Conclusion.** This work proposes structured pruning as an efficient method for creating competitive smaller-scale LLMs. Our two-stage approach combines targeted structured pruning and continued pre-training (continued pre-training), and we introduce _dynamic batch loading_ to improve pretraining data efficiency. We train a series of competitive Sheared-LLaMA models using a fraction

- f the compute required for standard pre-training. Our results show a promising path to producing
...
Brian R Bartoldson, Bhavya Kailkhura, and Davis Blalock. Compute-efficient deep learning: Algorithmic trends and opportunities. _Journal of Machine Learning Research_, 24:1–77, 2023.


Jason Baumgartner, Savvas Zannettou, Brian Keegan, Megan Squire, and Jeremy Blackburn. The
pushshift reddit dataset. _ArXiv_, abs/2001.08435, 2020.
...
Cody Blakeney, Jessica Zosa Forde, Jonathan Frankle, Ziliang Zong, and Matthew L Leavitt. Reduce, reuse, recycle: Improving training efficiency with distillation. _arXiv preprint_
_arXiv:2211.00683_, 2022.
...
Michael Carbin. The lottery ticket hypothesis for pre-trained bert networks. In _Advances in_
...
_arXiv preprint arXiv:2302.06675_, 2023.
...
acceleration for neural networks: A comprehensive survey. _Proceedings of the IEEE_, 108(4):
485–532, 2020.
...
Tim Dettmers, Artidoro Pagnoni, Ari Holtzman, and Luke Zettlemoyer. Qlora: Efficient finetuning
  ...
  - f quantized llms. _arXiv preprint arXiv:2305.14314_, 2023.
...
Elias Frantar and Dan Alistarh. Sparsegpt: Massive language models can be accurately pruned in
 ...
 - ne-shot, 2023. _arXiv preprint arXiv:2301.00774_, 2023.
...
bert by progressively stacking. In _International conference on machine learning_, pp. 2337–2346.
PMLR, 2019.
...
How to (re) warm your model? _arXiv preprint arXiv:2308.04014_, 2023.
...
_Science_, pp. 218–223, 2017.


Song Han, Huizi Mao, Dally, and William Dally. Deep compression: Compressing deep neural
networks with pruning, trained quantization and huffman coding. In _International Conference on_
...
bert with adaptive width and depth. _Advances in Neural Information Processing Systems_, 33:
9782–9793, 2020.


Ananya Harsh Jha, Dirk Groeneveld, Emma Strubell, and Iz Beltagy. Large language model distillation doesn’t need a teacher. _arXiv preprint arXiv:2305.14864_, 2023.
...
deep learning by focusing on the biggest losers. _arXiv preprint arXiv:1910.00762_, 2019.


Xiaoqi Jiao, Yichun Yin, Lifeng Shang, Xin Jiang, Xiao Chen, Linlin Li, Fang Wang, and Qun Liu.
Tinybert: Distilling bert for natural language understanding. In _Findings of the Association for_
_Computational Linguistics: EMNLP 2020_, pp. 4163–4174, 2020.


Jean Kaddour, Oscar Key, Piotr Nawrot, Pasquale Minervini, and Matt J Kusner. No train no gain:
Revisiting efficient training algorithms for transformer-based language models. _arXiv preprint_
_arXiv:2307.06440_, 2023.


Eldar Kurtic, Elias Frantar, and Dan Alistarh. Ziplm: Hardware-aware structured pruning of language models. _arXiv preprint arXiv:2302.04089_, 2023.


Tom Kwiatkowski, Jennimaria Palomaki, Olivia Redfield, Michael Collins, Ankur Parikh, Chris
Alberti, Danielle Epstein, Illia Polosukhin, Jacob Devlin, Kenton Lee, Kristina Toutanova, Llion
Jones, Matthew Kelcey, Ming-Wei Chang, Andrew M. Dai, Jakob Uszkoreit, Quoc Le, and Slav
Petrov. Natural questions: A benchmark for question answering research. _Transactions of the_
_Association for Computational Linguistics_, 7:452–466, 2019.


Franc¸ois Lagunas, Ella Charlaix, Victor Sanh, and Alexander M Rush. Block pruning for faster
transformers. _arXiv preprint arXiv:2109.04838_, 2021.
...
efficient convnets. In _International Conference on Learning Representations_, 2016.
...
you! _arXiv preprint arXiv:2305.06161_, 2023.


Zhuohan Li, Eric Wallace, Sheng Shen, Kevin Lin, Kurt Keutzer, Dan Klein, and Joey Gonzalez.
Train big, then compress: Rethinking model size for efficient training and inference of transformers. In _International Conference on machine learning_, pp. 5958–5968. PMLR, 2020.
...
challenge dataset for machine reading comprehension with logical reasoning. In _Proceedings of_
_the Twenty-Ninth International Joint Conference on Artificial Intelligence, IJCAI-20_, pp. 3622–
3628, 2020.
...
l ~~0~~ regularization. In _International Conference on Learning Representations_, 2018.
...
language models. _arXiv preprint arXiv:2305.11627_, 2023.
...
training on points that are learnable, worth learning, and not yet learnt. In _International Confer-_
_ence on Machine Learning_, pp. 15630–15649. PMLR, 2022.
...
instructions with human feedback. _Advances in neural information processing systems_, 35, 2022.
...
Word prediction requiring a broad discourse context. In _Proceedings of the 54th Annual Meeting_

_of the Association for Computational Linguistics (Volume 1: Long Papers)_, pp. 1525–1534, 2016.
...
John Platt and Alan Barr. Constrained differential optimization. In _Neural Information Processing_
_Systems_, 1987.
...
Victor Sanh, Thomas Wolf, and Alexander Rush. Movement pruning: Adaptive sparsity by finetuning. _Advances in Neural Information Processing Systems_, 33:20378–20389, 2020.


Noam M. Shazeer. Glu variants improve transformer. _ArXiv_, abs/2002.05202, 2020.
...
_arXiv preprint arXiv:2309.10818_, 2023.


Mingjie Sun, Zhuang Liu, Anna Bair, and J Zico Kolter. A simple and effective pruning approach
for large language models. _arXiv preprint arXiv:2306.11695_, 2023.


Zhiqing Sun, Hongkun Yu, Xiaodan Song, Renjie Liu, Yiming Yang, and Denny Zhou. Mobilebert:
a compact task-agnostic bert for resource-limited devices. In _Proceedings of the 58th Annual_
_Meeting of the Association for Computational Linguistics_, pp. 2158–2170, 2020.
...
Trieu H. Trinh and Quoc V. Le. A simple method for commonsense reasoning. _ArXiv_,
abs/1806.02847, 2018.
...
Zheyuan Bai, Yun Wang, et al. Pangu-pi: Enhancing language model architectures via nonlinearity compensation. _arXiv preprint arXiv:2312.17276_, 2023b.
...
_(EMNLP)_, pp. 6151–6162, 2020.


Johannes Welbl, Nelson F. Liu, and Matt Gardner. Crowdsourcing multiple choice science questions.
In _Proceedings of the 3rd Workshop on Noisy User-generated Text_, pp. 94–106, 2017.


Wei Wen, Chunpeng Wu, Yandan Wang, Yiran Chen, and Hai Li. Learning structured sparsity in
deep neural networks. _Advances in neural information processing systems_, 29, 2016.


Mengzhou Xia, Zexuan Zhong, and Danqi Chen. Structured pruning learns compact and accurate
models. In _Proceedings of the 60th Annual Meeting of the Association for Computational Lin-_
_guistics (Volume 1: Long Papers)_, pp. 1513–1528, Dublin, Ireland, May 2022. Association for
...
In _Proceedings of the 61st Annual Meeting of the Association for Computational Linguistics (Vol-_
_ume 1: Long Papers)_, pp. 13711–13738, Toronto, Canada, July 2023. Association for Computational Linguistics. doi: 10.18653/v1/2023.acl-long.767.
...
language model pretraining. _arXiv preprint arXiv:2305.10429_, 2023.
...
_for Computational Linguistics_, pp. 4791–4800, 2019.


Minjia Zhang and Yuxiong He. Accelerating training of transformer-based language models with
progressive layer dropping. _Advances in Neural Information Processing Systems_, 33:14011–
14023, 2020.


Peiyuan Zhang, Guangtao Zeng, Tianduo Wang, and Wei Lu. Tinyllama: An open-source small
language model. _arXiv preprint arXiv:2401.02385_, 2024.
...
using a hard concrete distribution introduced in Louizos et al. (2018). Given a masking variable _z_
parameterized by _α_, the hard concrete distribution is defined as follows:
...
_s_ ¯ = _s_ ( _ζ −_ _γ_ ) + _γ_
_z_ = min(1 _,_ max(0 _,_ ¯ _s_ ))


where _U_ is the uniform distribution, _β_ is a temperature parameter, _s_ is a relaxed binary mask that
conforms to the hard concrete distribution, and _ζ_ and _γ_ are the bounds of the hard concrete distribution. The hard concrete distribution serves as a continuous relaxation of the binary mask, allowing
...
(Wang et al., 2020; Xia et al., 2022). In our experiments, we set _β_ = 0 _._ 83, _ζ_ = 1 _._ 1, and _γ_ = _−_ 0 _._ 1.


To enforce the sparsity constraint, the masks are trained alongside with Lagrange multipliers _λ_,
...
[outputs is equivalent to removing the corresponding parameters. Please refer to composer](https://github.com/princeton-nlp/LLM-Shearing/blob/main/llmshearing/models/composer_llama.py) ~~l~~ lama.py
for more details.
...
The scaling law of language modeling is a function of model size _N_ and dataset size _D_ :
...
need a minimum of three models to estimate the constant _E_ + _D_ _[B][β]_ _[, A]_ [ and] _[ α]_ [. If the models are trained]

with different amount of tokens, we can estimate _E, A, α, B, β_ with a minimal of 5 models. Note
...
rate of 1 _._ 0 works well for pruning. We do not tune any other hyper-parameters. The throughput is
...
Training budget 0 _._ 4B 50B
Learning rate of _z, ϕ, λ_ 1 _._ 0        Learning Rate of _θ_ 0 _._ 0001 0 _._ 0001
LR warmup ratio 10% 3%
Batch size (tokens) 131K 1M
Evaluation interval _m_ (steps) 50 400
Steps 3 _,_ 200 51 _,_ 200
# GPUs 8 16
...
[10https://sharegpt.com. We only use the first round in the multi-turn chat history.](https://sharegpt.com)
...
- f the two rounds to eliminate the position bias.
...
Caption: illustrates the evolution of domain weights throughout the training process and the final
...
Caption: Left: Data weight of each batch during the continued pre-training stage. Right: Cumulative
...
global semiconductor market. This includes evaluating the performance
 ...
 - f individual companies, industries, and trends within the market.
...
- ur proposed targeted structured pruning. The total number of tokens for pruning and continued pretraining is controlled to be the same, and data from the RedPajama dataset is used directly without
...
Caption: The loss of LLM-Pruner and Sheared-LLaMA during the continued pre-training stage.
...
Caption: This section compares the performance of Sheared-LLaMA when trained with the scaling
...
Caption: Average downstream peformance of
...
Caption: The downstream performance of continued pre-training Pythia-160M and Sheared-Pythia160M. Sheared-Pythia-160M eventually outperforms the performance of continued pre-training
...
Caption: A comparison between pruning from
...
Caption: The loss of continued pre-training
...
|Col1|Cont' pre-training INCITE<br>Cont' pre-training our pruned model|Col3|Col4|
|---|---|---|---|
||Cont' pre-training INCITE<br>Cont' pre-training our pruned model|Cont' pre-training INCITE<br>Cont' pre-training our pruned model|Cont' pre-training INCITE<br>Cont' pre-training our pruned model|



### Figure 12

Caption: Average downstream performance

Meaning: The figure supports the method or empirical discussion described by this caption; consult the local source PDF for the visual encoding.

- f continuing pre-training Sheared-LLaMA vs
INCITE-Base-3B.



We used a learning rate 1 _e −_ 5 for continued pre-training INCITE-Base-3B, along with a scheduler
to warm up the learning rate to 1 _e −_ 5 in the first 3% of the training steps, and follows a cosine
...
Books, with a heuristically constructed proportion of 40% _,_ 40% _,_ 10% _,_ 10%, respectively. We then
...
- f loss difference across domains.
...
- f TinyLlama-1.1B is comparable to Sheared-LLaMA-1.3B on downstream benchmarks when used
...
- f each domain based on the model’s loss using an exponential ascent algorithm. However, dy
...
- ver, the structured pruning process itself clearly breaks this assumption. Nonetheless, this thought
